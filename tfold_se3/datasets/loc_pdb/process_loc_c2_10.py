...
# save path
valid_pdb_dpath = '/mnt/smile1/pdb/Deeploc_c2_10/v_pdb'
bcx_fpath = '/mnt/smile1/pdb/Deeploc_c2_10/sequence-clusters/bc-0.out'
chain_id_dpath = '/mnt/smile1/pdb/Deeploc_c2_10'
label_dpath = '/mnt/smile1/pdb/Deeploc_c2_10'
fasta_dpath = '/mnt/smile1/pdb/Deeploc_c2_10/fasta'
valid_num = 1108
seed = 42
...
print('c2_label_dict: ', c2_label_dict)
# input('debug')
...
with open(fasta_fpath) as f:
    ...
    for idx, line in enumerate(lines):
        if '>' in line:
            ...
            label_c2 = c2_label_dict[label_str]
            if 'test' in line:
                test_dict[chain_id] = '{},{}'.format(label, label_c2)
            else:
                non_test_dict[chain_id] = '{},{}'.format(label, label_c2)
                non_test_list.append(chain_id)
            all_dict[chain_id] = '{},{}'.format(label, label_c2)
...
with open(os.path.join(chain_id_dpath, 'loc_chain_ids_tst.txt'), 'w') as w_txt:
    with open(os.path.join(label_dpath, 'loc_tst_c2_10.csv'), 'w') as w_csv:
        for (key, value) in test_dict.items():
            w_txt.write('{}\n'.format(key))
            w_csv.write('{},{}\n'.format(key, value))

with open(os.path.join(chain_id_dpath, 'loc_chain_ids_trnval.txt'), 'w') as w_txt:
    with open(os.path.join(label_dpath, 'loc_trnval_c2_10.csv'), 'w') as w_csv:
        for (key, value) in non_test_dict.items():
            w_txt.write('{}\n'.format(key))
            w_csv.write('{},{}\n'.format(key, value))

with open(os.path.join(chain_id_dpath, 'loc_chain_ids_all.txt'), 'w') as w_txt:
    with open(os.path.join(label_dpath, 'loc_all_c2_10.csv'), 'w') as w_csv:
...
with open(os.path.join(chain_id_dpath, 'loc_chain_ids_val.txt'), 'w') as w_txt:
    with open(os.path.join(label_dpath, 'loc_val_c2_10.csv'), 'w') as w_csv:
        for key in valid_list:
            w_txt.write('{}\n'.format(key))
            w_csv.write('{},{}\n'.format(key, non_test_dict[key]))

with open(os.path.join(chain_id_dpath, 'loc_chain_ids_trn.txt'), 'w') as w_txt:
    with open(os.path.join(label_dpath, 'loc_trn_c2_10.csv'), 'w') as w_csv:
