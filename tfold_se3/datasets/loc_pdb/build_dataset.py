...
def build_hdf_file(bcx_data, pdb_dpath, hdf_fpath, log_fpath, fas_dpath, label_dict, label_c2_dict):
    ...
    for chain_name in bcx_data:
        status = None
        # for chain_name in chain_names:
            # pdb_code, chain_id = chain_name.split('_')
        pdb_code = chain_name
        label = label_dict[pdb_code]
        label_c2 = label_c2_dict[pdb_code]
        # chain_id = None
        chain_id = 'A'
        # pgz_fpath = os.path.join(
        #     pdb_dpath, pdb_code[1:-1].lower(), 'pdb%s.ent.gz' % pdb_code.lower())
        pdb_fpath = os.path.join(
            pdb_dpath, '{}.pdb'.format(pdb_code))

        fas_fpath = os.path.join(fas_dpath, '{}.fasta'.format(pdb_code))
        try:
            aa_seq, atom_cords, atom_masks, status = parser.run(pdb_fpath, chain_id)
            status_list.append((pdb_fpath, chain_id, status))

        except PDBConstructionException as pdb_error:
            raise PDBConstructionException('PDB: %s-%s' % (pdb_fpath, chain_id)) from pdb_error
        if status == PdbParserStatus.NORMAL:
            pdb_data[chain_name] = (aa_seq, atom_cords, atom_masks, label, label_c2)
        else:
            print(pdb_fpath)
            print('Status is NOT normal')
    ...
    with h5py.File(hdf_fpath, 'w') as o_file:
        for chain_name, (aa_seq, atom_cords, atom_masks, label, label_c2) in pdb_data.items():
            ...
            group.create_dataset('label_c2', data=label_c2)
...
def main():
    ...
    label_dict = {}
    label_c2_dict = {}
    with open(config['csv_fpath']) as f:
        for line in f:
            pdbid = line.split(',')[0].strip()
            label = line.split(',')[1].strip()
            label_c2 = line.split(',')[2].strip()
            label_dict[pdbid] = label
            label_c2_dict[pdbid] = label_c2
    ...
    for idx_file in range(n_files):
        ...
        args_list.append((bcx_data_sel, config['pdb_dpath_raw'], hdf_fpath, log_fpath, config['fas_ddpath'], label_dict, label_c2_dict))
